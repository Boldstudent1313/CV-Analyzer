...
def analyze_api():
    ...
    try:
        ...
        cv = (data.get('cv_text') or '').lower()
        job = (data.get('job_text') or '').lower()
        if not cv.strip() or not job.strip():
            return jsonify({'ok': False, 'error': 'cv_text and job_text are required'}), 400
        # Extract simple keyword set from job text
        tokens = re.findall(r"[a-zA-Z0-9+#.]{2,}", job)
        stop = set(['and','or','the','for','with','to','of','in','on','a','an','is','are','as','by','be','at','from','this','that','you','your'])
        kws = sorted({t for t in tokens if t not in stop})
        matched = [k for k in kws if k in cv]
        missing = [k for k in kws if k not in cv]
        # Score = coverage weighted by unique keywords
        score = int(round((len(matched) / max(1, len(kws))) * 100))
        recs = []
        if missing:
            recs.append(f"Consider adding evidence for: {', '.join(missing[:8])}.")
        if score < 60:
        ...
        if 'projects' not in cv and 'project' not in cv:
